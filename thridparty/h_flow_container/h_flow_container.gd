...
func _get_minimum_size() -> Vector2:
	...
	return Vector2(max_requested_width + horizontal_margin * 2, height + vertical_margin)
...
func _calculate_layout(_apply: bool) -> float:
	...
	var next_location: Vector2 = Vector2(horizontal_margin, vertical_margin)
	...
	for child in get_children():
		...
		if requested_size.x + next_location.x > rect_size.x:
			...
			next_location = Vector2(horizontal_margin, next_location.y + row_height + vertical_margin)
			row_height = 0
